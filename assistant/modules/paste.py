...
async def paste_bin(client, message):
    ...
    if len(message.command) > 1:
        ...
        if message.command[1].startswith("c") or message.command[1].startswith("objc"):
            extension = "c"
        if (
            message.command[1].startswith("json")
            or message.command[1].startswith("yaml")
            or message.command[1].startswith("toml")
        ):
            extension = "json"
        if message.command[1].startswith("markdown") or message.command[1].startswith(
            ...
            extension = "markdown"
        if (
            message.command[1].startswith("html")
            or message.command[1].startswith("css")
            or message.command[1].startswith("xml")
        ):
            extension = "markup"
